...
namespace TemplateProcess {
  ...
  export function make() {
    ...
    for (const request of responses_data) {
      let career_name = request.datas.career
        .split("(")[1]
        .replace(")", "")
        .trim();
      ...
      let document = copy_template(
        `${marks["enrollment"]} - ${marks["name"]} - ${request.datas.datetime}`
      );
   ...
   */
  function data_base_to_marks(
    request: {},
    career_data: {},
    directors_data: {}[],
    base_marks = SETTINGS.MARKS
  ) {
    ...
    for (const dir of directors_data) {
      if (dir.datas.area === SETTINGS.DIRECTOR_AREA.academic) {
        marks["academic_director"] = dir.datas.name;
      } else if (dir.datas.area === SETTINGS.DIRECTOR_AREA.admin) {
        marks["administrative_director"] = dir.datas.name;
      }
    }
      ...
      if (datas[`${career}1`]) {
        for (let i = 0; i < SETTINGS.SUBJECTS_BY_CAREER; i++) {
          ...
          if (subject_data.length > 1) {
            subjects[`id${i + 1}`] = subject_data[0]
              ? subject_data[0].trim()
              : "";
            subjects[`subject${i + 1}`] = subject_data[1]
              ? subject_data[1].trim()
              : "";
          } else {
            subjects[`id${i + 1}`] = "";
            subjects[`subject${i + 1}`] = subject_data[0]
              ? subject_data[0].trim()
              : "";
          }
        }
  ...
  function copy_template(
    ...
    let document = DriveApp.getFileById(SETTINGS.FORMAT_TEMPLATE_ID).makeCopy(
      file_name,
      folder
    );
   ...
   */
  function template_data_reemplace(
